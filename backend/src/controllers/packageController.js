...
exports.create = async (req, res) => {
  try {
    const { packageName, packageImageCount, packageResolution, packageDeliveryDays, packagePrice } = req.body

    if (!packageName?.trim() || !packageImageCount || !packageResolution || !packageDeliveryDays || packagePrice === undefined) {
      return res.status(400).json({ message: 'กรุณากรอกข้อมูลให้ครบถ้วน' })
    }

    if (!VALID_RESOLUTIONS.includes(packageResolution)) {
      return res.status(400).json({ message: 'packageResolution ต้องเป็น FullHD หรือ 4K' })
    }

    const [result] = await PackageModel.create({ ...req.body, packageName: packageName.trim() })
