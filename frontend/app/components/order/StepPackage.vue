  ...
  <div class="p-6 sm:p-8">
    ...
    >
      <button
        ...
        :key="pkg.packageId"
        class="group relative flex min-h-[340px] flex-col rounded-[16px] border bg-white/80 p-6 text-left transition duration-200 hover:-translate-y-px hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-black/20"
        :class="
      ...
      >
        ...
        <h3 class="mb-1.5 pr-8 text-[14px] font-bold text-[#171717]">
          {{ pkg.packageName }}
        </h3>
        
        <div class="mb-5">
          <span class="text-[24px] font-bold text-[#171717]">฿{{ formatPrice(pkg.packagePrice) }}</span>
        </div>

        <ul class="flex-1 space-y-3 text-[13px] font-medium text-[#666666]">
          <li class="flex items-start gap-2.5">
            <svg class="mt-0.5 h-4 w-4 shrink-0 text-[#171717]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
          ...
          <li v-if="pkg.packageUrgentPrice" class="flex items-start gap-2.5">
            <svg class="mt-0.5 h-4 w-4 shrink-0 text-[#171717]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            <span>เร่งด่วน +฿{{ formatPrice(pkg.packageUrgentPrice) }}</span>
          </li>
          <li class="flex items-start gap-2.5">
            <svg class="mt-0.5 h-4 w-4 shrink-0 text-[#171717]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.99 1.99 0 013 12V7a4 4 0 014-4z"/></svg>
            <span>ลด {{ pkg.packageGalleryDiscount }}% หากโชว์ Gallery</span>
          </li>
        </ul>

        <p v-if="pkg.packageDescription" class="mt-5 border-t border-black/5 pt-4 text-[12px] font-medium leading-relaxed text-[#929292]">
