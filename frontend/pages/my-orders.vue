    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight">📁 ประวัติออเดอร์ของฉัน</h1>
      ...
      <!-- Filters Tab -->
      <div class="bg-white rounded-xl shadow-sm p-2 mb-6 flex flex-wrap gap-1 border border-gray-100 overflow-x-auto">
        <button
      ...
      <!-- Loading State -->
      <div v-if="loading" class="bg-white rounded-2xl shadow-md p-12 text-center border border-gray-100">
        <div class="animate-spin w-10 h-10 border-4 border-indigo-200 border-t-indigo-600 rounded-full mx-auto mb-4"></div>
        <p class="text-gray-500 font-medium">กำลังดึงข้อมูลออเดอร์ของคุณ...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="bg-white rounded-2xl shadow-md p-8 text-center border border-red-100">
        ...
        <button @click="fetchOrders" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-4 py-2 rounded-xl transition text-sm">
          โหลดอีกครั้ง
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredOrders.length === 0" class="bg-white rounded-2xl shadow-md p-12 text-center border border-gray-100">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          <div class="px-6 py-4 bg-gray-50/50 border-b border-gray-100 flex items-center justify-between">
            <span class="font-bold text-gray-700">ออเดอร์ #{{ order.orderId }}</span>
            <span
          ...
          <div class="p-6 flex-1 flex flex-col justify-between">
            <div class="space-y-3">
              ...
              <div class="flex justify-between gap-4">
                <div>
                  ...
                  <span class="text-gray-700 font-medium text-sm">📦 {{ order.packageName }}</span>
                </div>
                <div>
                  <span class="text-xs text-gray-400 font-semibold uppercase tracking-wider block">วันที่ต้องการงาน</span>
                  <span class="text-gray-700 font-medium text-sm">📅 {{ formatDate(order.orderRequiredDate) }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
              <div>
                ...
                <span class="text-xl font-black text-indigo-600">฿{{ formatPrice(order.orderTotalPrice) }}</span>
              </div>
              <span class="text-sm font-semibold text-indigo-600 group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                ดูรายละเอียด
